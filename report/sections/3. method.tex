...
This tradeoff is inherent: to minimize transfer time one would ideally store all inputs on the GPU, but memory constraints make this impossible for realistic $n$ and $m$. Even storing everything in CPU RAM is nontrivial at scale, so the entire computational pipeline must be rethought. The paper proposes an implementation that maximizes compute density on GPUs while keeping memory usage—and therefore data transfers—under control. 
...
The full algorithm is explicitly written in Algorithm 1. The multi GPU advantage is exploited by distributing different block-rows of the matrix across GPUs, allowing them to process independent parts of the Cholesky decomposition in parallel.
\begin{algorithm}[h]
\caption{Block Cholesky Decomposition (In-Place)}
\KwIn{$K_{mm}$ noted $K^{(m)}$, split into $B \times B$ tiles $K^{(m)}_{ij} \in \mathbb{R}^{t \times t}$.}
\KwOut{Lower triangular matrix $L$ such that $K^{(m)} = LL^\top$.}

\For{$j = 1$ \KwTo $B$}{
    $L_{j,j} \leftarrow \mathrm{chol}(A_{j,j})$      
    \For{$i = j+1$ \KwTo $B$}{
        $L_{i,j} \leftarrow K^{(m)}_{i,j} (L_{j,j}^{-T})$  
    }
    \For{$i = j+1$ \KwTo $B$}{
        \For{$k = j+1$ \KwTo $i$}{
            $K^{(m)}_{i,k} \leftarrow K^{(m)}_{i,k} - L_{i,j} L_{k,j}^\top$  
        }
    }
}
\end{algorithm}
